...
class RobertaForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config, do_not_summary):
        super(RobertaForMultipleChoice, self).__init__(config)
        self.num_labels = config.num_labels

        self.roberta = RobertaModel(config)
        self.classifier = RobertaClassificationHead(config, 1, do_not_summary)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
                position_ids=None, head_mask=None, task_id=None):
...
class RobertaForMultipleChoice_MT(BertPreTrainedModel):
    """
    Roberta Model with a classifier head on top.
    """
    config_class = RobertaConfig
    ...
    def __init__(self, config, num_labels, do_not_summary):
        super(RobertaForMultipleChoice_MT, self).__init__(config)
        self.num_labels = num_labels

        self.roberta = RobertaModel(config)
        self.classifiers = nn.ModuleList([RobertaClassificationHead(config, 1, do_not_summary),
                                          RobertaClassificationHead(config, 1, do_not_summary)])
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        logits = logits.view(-1, self.num_labels[task_id])
...
class RobertaForMultipleChoice_MT_general(BertPreTrainedModel):
    """
    Roberta Model with a classifier head on top.
    """
    config_class = RobertaConfig
    ...
    def __init__(self, config, task_output_config, do_not_summary, same_linear_layer):
        super(RobertaForMultipleChoice_MT_general, self).__init__(config)
        self.roberta = RobertaModel(config)

        self.classifiers = []
        self.output_modes = [m for m, n in task_output_config]
        self.num_labels = [n for m, n in task_output_config]
        self.task_id_map = {}
        if same_linear_layer:
            multi_choice_index = None
            for idx, (output_mode, num_label) in enumerate(task_output_config):
                if output_mode == 'multi-choice':
                    if multi_choice_index is not None:
                        self.task_id_map[idx] = multi_choice_index
                    else:
                        self.classifiers.append(RobertaClassificationHead(config, 1, do_not_summary))
                        self.task_id_map[idx] = idx
                        multi_choice_index = idx
                else:
                    self.classifiers.append(RobertaClassificationHead(config, num_label, do_not_summary))
                    self.task_id_map[idx] = max(self.task_id_map.values()) + 1 \
                        if self.task_id_map.values() else len(self.task_id_map)
        else:
            for idx, (output_mode, num_label) in enumerate(task_output_config):
                if output_mode == 'multi-choice':
                    self.classifiers.append(RobertaClassificationHead(config, 1, do_not_summary))
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        outputs = self.roberta(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                               attention_mask=attention_mask, head_mask=head_mask)
        sequence_output = outputs[0]
        logits = self.classifiers[self.task_id_map[task_id]](sequence_output)
        if self.output_modes[task_id] == 'multi-choice':
            logits = logits.view(-1, self.num_labels[task_id])
